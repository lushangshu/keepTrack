...
@interface PieGraphPieceData : NSObject
...
@implementation PieGraphPieceData
@end


@interface PieGraphChart () {
    CGPoint moveP;
}
@property (nonatomic, retain) NSMutableArray *chartsData;
@property (nonatomic) float radius;
@property (nonatomic) float holeRadius;
@property (nonatomic, assign) PieGraphPiece *hitLayer;
@property (nonatomic) BOOL presentWithAnimation;
@property (nonatomic) PieGraphChartAnimationOptions animationOptions;
@property (nonatomic) float animationDuration;

@end

@interface PieGraphPiece ()
@property (nonatomic, copy) void (^endAnimationBlock)(void);
- (void) _animate;
- (void) setAnimationOptions:(PieGraphChartAnimationOptions)options;
- (void) setAnimationDuration:(float)duration;
- (void) setShowLabel:(BOOL)value;
@end


@implementation PieGraphChart {
    CGPoint _touchBegan;
}
...
- (id) init {
    ...
    self.strokeColor = [UIColor colorWithRed:0.9 green:0.9 blue:0.9 alpha:1];
    self.startAngle = 0;
    self.length = M_PI*2;
    self.radiusPrecent = 0.58;
    self.holeRadius = 0;
...
- (void) updateCharts {
    ...
    for (PieGraphPieceData *data in _chartsData) {
        ...
        PieGraphPiece *piece = [[PieGraphPiece alloc] init];
        [piece setFrame:rect];
        if (data.accent) {
    ...
    if (_presentWithAnimation) {
        ...
        if (_animationOptions & PieGraphChartAnimationGrowthAll || _animationOptions & PieGraphChartAnimationGrowthBackAll || _animationOptions & PieGraphChartAnimationFan) {
            
            for (NSInteger i = 0, len = [self.layer sublayers].count; i < len; i++) {
                PieGraphPiece *piece = [[self.layer sublayers] objectAtIndex:i];
                [piece _animate];
            }
            ...
            for (NSInteger i = 0, len = [self.layer sublayers].count; i < len; i++) {
                PieGraphPiece *piece = [[self.layer sublayers] objectAtIndex:i];
                if (i+1 < len) {
                    __block PieGraphPiece *blockPiece = [[self.layer sublayers] objectAtIndex:i+1];
            ...
            PieGraphPiece *piece = [[self.layer sublayers] objectAtIndex:0];
            [piece _animate];
        }
...
- (void) setChartValues:(NSArray *)chartValues animation:(BOOL)animation {
    [self setChartValues:chartValues animation:animation options:(PieGraphChartAnimationFanAll | PieGraphChartAnimationTimingLinear)];
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    _hitLayer = (PieGraphPiece*)[self layerForTouch:touch];
...
- (void) touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if ([_hitLayer isKindOfClass:[PieGraphPiece class]]) {
