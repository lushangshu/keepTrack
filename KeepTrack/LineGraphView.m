...
- (void)layoutSubviews{
    ...
    self.animationDelegate = [[LineGraphAnimation alloc] init];
...
- (LineGraphCircle *)closestDotFromVerticalLine:(UIView *)verticalLine {
    currentlyCloser = 1000;
    
    for (LineGraphCircle *dot in self.subviews) {
...
- (void)drawGraph {
    ...
    for (UIView *subview in [self subviews]) {
        if ([subview isKindOfClass:[LineGraphCircle class]])
            [subview removeFromSuperview];
    }
    ...
    for (int i = 0; i < numberOfPoints; i++) {
        ...
        if (positionOnYAxis!=positionOnYAxis) {
            positionOnYAxis=self.viewForBaselineLayout.frame.size.height;
        }
        
        LineGraphCircle *circleDot = [[LineGraphCircle alloc] initWithFrame:CGRectMake(0, 0, circleSize, circleSize)];
    ...
    for (UIView *subview in [self subviews]) {
        if ([subview isKindOfClass:[LineGraphLine class]])
            [subview removeFromSuperview];
    }
    ...
    for (int i = 0; i < numberOfPoints - 1; i++) {
        ...
        LineGraphLine *line = [[LineGraphLine alloc] initWithFrame:CGRectMake(0, 0, self.viewForBaselineLayout.frame.size.width, self.viewForBaselineLayout.frame.size.height)];
        line.opaque = NO;
        line.tag = i + 1000;
        line.alpha = 0;
