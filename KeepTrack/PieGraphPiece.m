...
- (void) createArcAnimationForKey:(NSString *)key fromValue:(NSNumber *)from toValue:(NSNumber *)to delegate:(id)delegate {
    ...
    if (_animationOptions & PieGraphChartAnimationFanAll) {
        arcAnimation.duration = _animationDuration/((M_PI*2)/_angle);
    } else if (_animationOptions & PieGraphChartAnimationGrowth || _animationOptions & PieGraphChartAnimationGrowthBack) {
        arcAnimation.duration = _animationDuration/(float)[[self.superlayer sublayers] count];
    } else if (_animationOptions & PieGraphChartAnimationGrowthAll || _animationOptions & PieGraphChartAnimationGrowthBackAll) {
        arcAnimation.duration = _animationDuration;
    } else if (_animationOptions & PieGraphChartAnimationFan) {
        arcAnimation.duration = _animationDuration;
    }
    
    [arcAnimation setDelegate:delegate];
    
    if (_animationOptions & PieGraphChartAnimationTimingEaseIn) {
        [arcAnimation setTimingFunction:[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseIn]];
    } else if (_animationOptions & PieGraphChartAnimationTimingEaseOut) {
        [arcAnimation setTimingFunction:[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseOut]];
    } else if (_animationOptions & PieGraphChartAnimationTimingEaseInOut) {
...
- (void) drawInContext:(CGContextRef)ctx {
    ...
    if (arcAnimation) {
        _angle = _endAngle;
        PieGraphPiece *p = arcAnimation.delegate;
        [p __angle:_endAngle];
    }
...
- (void) _animate {
    if (_animationOptions & PieGraphChartAnimationFan || _animationOptions & PieGraphChartAnimationFanAll) {
    ...
    if (_animationOptions & PieGraphChartAnimationGrowthBack || _animationOptions & PieGraphChartAnimationGrowthBackAll) {
    ...
    if (_animationOptions & PieGraphChartAnimationGrowth || _animationOptions & PieGraphChartAnimationGrowthAll) {
...
- (NSString *) description {
    return [NSString stringWithFormat:@"<PieGraphPiece: %p, _startAngle=%f, _endAngle=%f>", self, _startAngle, _endAngle];
}
